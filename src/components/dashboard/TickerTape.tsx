...
export function TickerTape() {
  ...
  const fetchTickers = async () => {
    try {
      ...
      if (data.authenticated && data.indices?.length > 0) {
        const mapped = data.indices.map((idx: any) => ({
          symbol: idx.name,
          ltp: idx.value,
  ...
  if (!mounted) return <div style={{ height: 36, background: 'var(--bg-secondary)', borderBottom: '1px solid var(--border)' }} />

  return (
    <div style={{ background: 'var(--bg-secondary)', borderBottom: '1px solid var(--border)', height: 36, fontSize: 12, overflow: 'hidden', whiteSpace: 'nowrap', display: 'flex', alignItems: 'center' }}>
      <div style={{ display: 'flex', alignItems: 'center', gap: 6, padding: '0 12px', flexShrink: 0, borderRight: '1px solid var(--border)', height: '100%' }}>
        <span className="live-dot" />
        <span style={{ color: 'var(--up)', fontSize: 10, fontWeight: 600, letterSpacing: '0.1em' }}>LIVE</span>
      </div>
      <div style={{ overflow: 'hidden', flex: 1 }}>
        <div style={{ display: 'inline-flex', alignItems: 'center', animation: 'ticker 60s linear infinite' }}>
          {[...tickers, ...tickers].map((ticker, i) => (
            <div key={i} style={{ display: 'flex', alignItems: 'center', gap: 8, padding: '0 16px', borderRight: '1px solid var(--border)', height: 36, flexShrink: 0 }}>
              ...
              <span style={{ color: ticker.change >= 0 ? 'var(--up)' : 'var(--down)', display: 'flex', alignItems: 'center', gap: 2, fontWeight: 500 }}>
                {ticker.change >= 0 ? <TrendingUp size={10} /> : <TrendingDown size={10} />}
